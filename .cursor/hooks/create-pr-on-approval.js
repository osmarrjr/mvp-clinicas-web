...
if (branch) {
  const prCount = run(`gh pr list --head "${branch}" --json number --jq "length"`);
  if (parseInt(prCount, 10) > 0) noop();
}

// 5. Extrai seções do plano.md para montar a mensagem de followup
const titulo = (conteudo.match(/^# Plano:\s*(.+)$/m) || [])[1] || 'implementação';
...
const qaSummary = qaResult.summary || 'Todos os testes E2E passaram.';
...
const followup = `\
✅ O QA aprovou a implementação — testes E2E passando e checklist do plano.md completo.
